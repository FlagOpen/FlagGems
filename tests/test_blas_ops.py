...
def test_accuracy_addmm(M, N, K, scalar, dtype):
    mat1 = torch.randn((M, K), dtype=dtype, device="cpu")
    mat2 = torch.randn((K, N), dtype=dtype, device="cpu")
    bias1 = torch.randn((N,), dtype=dtype, device="cpu")
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)
    ref_bias1 = to_reference(bias1, True)
    mat1 = mat1.to(flag_gems.device)
    mat2 = mat2.to(flag_gems.device)
    bias1 = bias1.to(flag_gems.device)
    ...
    bias2 = bias2.to(flag_gems.device)
...
def test_accuracy_bmm(M, N, K, dtype):
    batch = 4
    mat1 = torch.randn((batch, M, K), dtype=dtype, device="cpu")
    mat2 = torch.randn((batch, K, N), dtype=dtype, device="cpu")
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)
    mat1 = mat1.to(flag_gems.device)
    mat2 = mat2.to(flag_gems.device)
...
def test_accuracy_mv(M, N, dtype):
    matrix = torch.randn((N, M), dtype=dtype, device="cpu")
    vector = torch.randn((M,), dtype=dtype, device="cpu")
    ref_matrix = to_reference(matrix, True)
    ref_vector = to_reference(vector, True)
    matrix = matrix.to(flag_gems.device)
    vector = vector.to(flag_gems.device)
...
def test_accuracy_outer(M, N, dtype):
    inp1 = torch.randn(M, dtype=dtype, device="cpu", requires_grad=True)
    inp2 = torch.randn(N, dtype=dtype, device="cpu", requires_grad=True)
...
def test_accuracy_vdot(M, is_conj, dtype, stride):
    ...
    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)
    ref_inp1 = ref_inp1.to(flag_gems.device)
    ref_inp2 = ref_inp2.to(flag_gems.device)
