...
def test_accuracy_multinomial_without_replacement(pool, dtype):
    pytest.skip("test too long wait for fix")
...
@pytest.mark.stack
@pytest.mark.parametrize("shape", STACK_SHAPES + CAMBRICON_STACK_SHAPES)
...
CAMBRICON_VSTACK_SHAPES = [[(16, 128, 64, 64),
                            (16, 128, 64, 64),
                            (16, 128, 64, 64),
                            (16, 128, 64, 64)],
                           [(32, 64, 128, 8),
...
@pytest.mark.vstack
@pytest.mark.parametrize("shape", VSTACK_SHAPES + CAMBRICON_VSTACK_SHAPES)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES + INT_DTYPES)
...
def test_accuracy_diag(shape, diagonal, dtype):
    if dtype in FLOAT_DTYPES:
        ...
        inp = torch.randint(0, 0x7FFF, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_diag_embed(shape, dtype, offset, dim1, dim2):
    if dtype in FLOAT_DTYPES:
        ...
        inp = torch.randint(low=0, high=0x7FFF, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
    else:
        inp = torch.randint(low=0, high=2, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
