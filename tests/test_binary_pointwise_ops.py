...
def test_accuracy_clamp_tensor(shape, isnone, dtype):
    ...
    ) if ref_mini != None else None), max=(ref_maxi.cpu() if ref_maxi != None else None)).to(DEVICE)
    with flag_gems.use_gems():
        res_out = torch.clamp(inp, min=mini, max=maxi)
...
def test_accuracy_isclose(shape, dtype, zero_tol, equal_nan, gen_nan):
    rtol = (
        torch.rand(1, dtype=torch.float32, device=DEVICE).item() * 0.0001
        if not zero_tol
        else 0
    )
    if dtype in ALL_FLOAT_DTYPES:
        ...
        atol = (
            torch.finfo(dtype).tiny * torch.randint(0, 4, (1,), device=DEVICE).item()
            if not zero_tol
            else 0
        )
    else:
        inp1 = torch.randint(-1000, 1000, shape, device=DEVICE).to(dtype)
        inp2 = torch.randint(-1000, 1000, shape, device=DEVICE).to(dtype)
        if dtype in [torch.int64]:
            ...
            atol = (
                (
                    ...
                    * torch.randint(0, 10, (1,), device=DEVICE).item()
                )
                if not zero_tol
                else 0
            )
...
def test_accuracy_allclose(shape, dtype, equal_nan, gen_nan):
    rtol = torch.rand(1, dtype=torch.float32, device=DEVICE).item() * (
        0.0001 if dtype in [torch.bfloat16, torch.float16] else 0.01
    )
    if dtype in ALL_FLOAT_DTYPES:
        atol = torch.finfo(dtype).tiny * torch.randint(0, 4, (1,), device=DEVICE).item()
        inp1 = torch.full(shape, 1.234, dtype=dtype, device=DEVICE)
        inp2 = torch.full(shape, 1.234, dtype=dtype, device=DEVICE)
        if gen_nan:
            nan_num = torch.full(
                (1,),
                float("nan" if gen_nan == 1 else "inf"),
