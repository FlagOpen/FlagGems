...
class LibEntry(triton.KernelInterface):
    def __init__(
        ...
        self.specialize_indices = [
            ...
            if num not in self.jit_function.constexprs
            and num not in self.jit_function.do_not_specialize
        ]
        self.do_not_specialize_indices = [
            num
            for num, arg_name in enumerate(self.jit_function.arg_names)
            if num not in self.jit_function.constexprs
            and num in self.jit_function.do_not_specialize
        ]
    ...
    def run(self, *args, **kwargs):
        ...
        while entry_key not in cache:
            ...
            with self.lock:
                ...
                for num, arg_name in enumerate(self.jit_function.arg_names):
                    if (
                        num in self.jit_function.constexprs
                        and arg_name not in constexprs
                        and self.jit_function.arg_defaults[num] is not inspect._empty
                    ):
