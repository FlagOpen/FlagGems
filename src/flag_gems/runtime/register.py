...
class Register:
    def __init__(self, config, user_unused_ops_list=None, cpp_patched_ops_list=None, lib=None):
        ...
        self.cpp_patched_ops_list = cpp_patched_ops_list if cpp_patched_ops_list else []
        self.config = config
        self.config_filter()
        self.for_each()
    ...
    def config_filter(self):
        self.config = [
            item for item in self.config if item[1].__name__ not in self.unused_ops and item[0] not in self.cpp_patched_ops_list
        ]
