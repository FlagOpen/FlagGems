...
def local_quick_unique_flat_impl(
    ...
    if return_counts:
        ...
        tl.store(
            local_unique_ptr + (offset + local_unique_offset), 
            a, 
            mask=store_mask
        )
...
def global_cumsum_flat_impl(
    ...
    tile_size: tl.constexpr,
    return_counts: tl.constexpr,
    MAX_CTAS_NUM: tl.constexpr,  # 新增参数
):
    offset = global_pid * tile_size
    r = tl.arange(0, tile_size)
    i0 = offset + r
    ...
    # load tile_sum, 使用固定大小避免UB overflow
    start_idx = tl.maximum(global_pid - ctas_num, 0)
    end_idx = tl.minimum(global_pid, global_ctas_num)
    
    # 使用MAX_CTAS_NUM作为constexpr上界
    p = tl.arange(0, MAX_CTAS_NUM)
    p_idx = start_idx + p
    pre_tile_sum_mask = (p_idx < end_idx) & (p_idx >= 0) & (p_idx < global_ctas_num) & (p < ctas_num)
...
def global_cumsum_flat_kernel(
    ...
    pid = tle.program_id(0)
    ctas_num = tle.num_programs(0)
    MAX_CTAS_NUM: tl.constexpr = 8192  # 定义最大值
