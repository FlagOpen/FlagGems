...
def group_norm_kernel(
    ...
    channel_offsets = group_start_channel + tl.arange(0, BLOCK_GROUP_SIZE)
    channel_mask = channel_offsets < min(group_start_channel + group_size, C)
    
    num_elements = group_size * HW
    
    # 预加载 weight 和 bias
    if W is not None:
        weight = tl.load(W + channel_offsets, mask=channel_mask, other=1.0)[:, None]
    else:
        weight = 1.0
   
    if B is not None:
        bias = tl.load(B + channel_offsets, mask=channel_mask, other=0.0)[:, None]
    else:
        bias = 0.0
    ...
    for hw_start in range(0, HW, BLOCK_SUB_HW_SIZE):
        hw_size = min(BLOCK_SUB_HW_SIZE, HW - hw_start)
