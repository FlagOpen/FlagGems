...
def l2_norm_kernel_2(Mid, Out, MID_SIZE, BLOCK_MID: tl.constexpr, BLOCK_MID_SUB: tl.constexpr):
    pid = tl.program_id(0).to(tl.int64)
    base_offset = pid * BLOCK_MID

    # 初始化累加器
    total_sum = 0.0

    # 计算需要处理的总块数
    num_sub_blocks = BLOCK_MID // BLOCK_MID_SUB

    # 循环处理每个子块
    for sub_block_idx in range(num_sub_blocks):
        sub_offset = base_offset + sub_block_idx * BLOCK_MID_SUB
        offsets = sub_offset + tl.arange(0, BLOCK_MID_SUB)
        mask = offsets < MID_SIZE

        x = tl.load(Mid + offsets, mask=mask, other=0.0).to(tl.float32)
        sub_sum = tl.sum(x)  # 计算当前子块的和
        total_sum += sub_sum # 累积到总和中
...
def vector_norm(x, ord=2, dim=None, keepdim=False, dtype=None):
    ...
    with torch_device_fn.device(x.device):
        if (not dim) or len(dim) == x.ndim:
            ...
            BLOCK_SIZE2 = triton.next_power_of_2(math.ceil(math.sqrt(MID_SIZE)))
            CORE_NUM = triton.cdiv(MID_SIZE, BLOCK_SIZE2)
