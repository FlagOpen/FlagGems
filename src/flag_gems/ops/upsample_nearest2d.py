...
def configs2():
    ...
    num_stage = [1, 3]
    return [
        triton.Config({"BLOCK_H": bh}, num_warps=1, num_stages=s) for s in num_stage for bh in block_h
    ]
...
def upsample_nearest2d_kernel_opt(
    ...
    for idx in range((nc_end - nc_begin) * loop_num_h):
        ...
        tmp = data.reshape(BLOCK_H // 2, OW // 2, 1).broadcast_to(BLOCK_H // 2, OW // 2, 2).reshape(BLOCK_H // 2, 1, OW)
