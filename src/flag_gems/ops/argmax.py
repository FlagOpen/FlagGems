...
def argmax_kernel_non_inner(
    ...
    max_values = tl.full([BLOCK_K], dtype=acc_type, value=min_value)
    argmax_values = tl.full([BLOCK_K], dtype=tl.int64, value=0)

    loop_time = N // BLOCK_N
    remainder = N % BLOCK_N
    for start_n in range(0, loop_time):
        n_offset = start_n * BLOCK_N + tl.arange(0, BLOCK_N)
    ...
    if remainder:
        n_offset = loop_time * BLOCK_N + tl.arange(0, BLOCK_N)
        ...
        argmax_values = tl.where(
            update, loop_time * BLOCK_N + local_argmax, argmax_values
        )
