...
def radix_type_convert(k):
    ...
    elif tl.constexpr(k.dtype == tl.float16):
        ik = k.to(tl.int16, bitcast=True)
        mask = (ik >> 15) & 0x1
        o = tl.where(mask, ik & 0x7FFF, ik | 0x8000)
    elif tl.constexpr(k.dtype == tl.bfloat16):
        ik = k.to(tl.int16, bitcast=True)
        mask = (ik >> 15) & 0x1
        o = tl.where(mask, ik & 0x7FFF, ik | 0x8000)
    ...
    elif tl.constexpr(k.dtype == tl.float32):
        ik = k.to(tl.int32, bitcast=True)
        mask = (ik >> 31) & 0x1
        o = tl.where(mask, ik & 0x7FFFFFFF, ik | 0x80000000)
    ...
    elif tl.constexpr(k.dtype == tl.float64):
        ik = k.to(tl.int64, bitcast=True)
        mask = (ik >> 63) & 0x1
        o = tl.where(mask, ik & 0x7FFFFFFFFFFFFFFF, ik | 0x8000000000000000)
    else:
...
def sort_by_key(key, value, valid_bits):
    ...
    for p in range(passes):
        ...
        for portion_id in range(num_portions):
            portion_items = min(n_elements - portion_id * portion_size, portion_size)
...
def sort(self, dim=-1, descending=False):
    bsz = self.shape[0]
    last_dim_size = self.shape[-1]
    index = [i for i in range(last_dim_size)] * bsz
    ...
    # TODO(zhengzekang): temp process.
    index_tensor = torch.tensor(index, dtype=key_dtype, device=self.device)
    index_tensor = index_tensor.view(bsz, last_dim_size)
    # print(index_tensor)
    # print("self shape is: ", self.shape)
    # print("index tensor shape is: ", index_tensor.shape)

    sorted_key, sorted_value = sort_by_key(self, index_tensor, valid_bits)

    return sorted_key, sorted_value
