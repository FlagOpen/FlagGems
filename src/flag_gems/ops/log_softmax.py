...
MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_INNER = 18560
MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_NON_INNER = 20480
...
def config_prune1(configs, named_args, **kwargs):
    ...
    configs_map = {}
    pruned_configs = []
    # When N is less than MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_NON_INNER, no reduction loops
    doopt = N < MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_NON_INNER
    for config in configs:
        ...
        if doopt:
            ...
            if nram_usage < MAX_NRAM_SIZE:
                ...
                max_block_k = MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_NON_INNER // N
                align_num = 64 // 4
...
def log_softmax_tile_mode1(args):
...
@triton.heuristics(
    values={
        "TILE_MODE": lambda args: log_softmax_tile_mode1(args),
    }, )
@triton.jit
...
def config_prune2(configs, named_args, **kwargs):
    ...
    configs_map = {}
    pruned_configs = []
    # When N is less than MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_INNER, no reduction loops
    doopt = N < MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_INNER
    for config in configs:
        kw = config.kwargs
        ...
        if doopt:
            ...
            if nram_usage < MAX_NRAM_SIZE:
                ...
                max_block_m = MAX_MXN_LOG_SOFTMAX_FORWARD_FOR_INNER // N
                align_num = 64 // 4
...
def log_softmax_tile_mode2(args):
...
@triton.heuristics(
    values={
        "TILE_MODE": lambda args: log_softmax_tile_mode2(args),
    }, )
@triton.jit
...
@triton.autotune(
    configs=[
        triton.Config({"TILE_M": 2**k}, num_warps=1, num_stages = s)
...
@triton.heuristics(
    values={
        "TILE_N": lambda args: max(1, MAX_C_MLU_LOG_SOFTMAX_BACKWARD // args["TILE_M"]),
        "ONE_TILE_PER_CTA": lambda args: args["TILE_N"] >= args["N"],
    },
)
@triton.jit
