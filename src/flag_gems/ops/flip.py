...
def flip_origin(A: torch.Tensor, dims) -> torch.Tensor:
...
def {wrapper_name}(x, merge_shapes, merge_strides, merge_dim_size):
    ...
    def __config(self):
        ...
        # generate config key.
        merge_shapes_args_str = ", ".join([f"'merge_shape_{i}'" for i in range(self.merge_dim_size)])
        merge_strides_args_str = ", ".join([f"'merge_stride_{i}'" for i in range(self.merge_dim_size)])
...
@triton.jit
    ...
    def __kernel_flip_2d(self):
        ...
        self.writeline(f"src_offset += pid_y * step * merge_shape_{self.merge_dim_size - 1}")
        if self.merge_flip_dims_flags[self.merge_dim_size - 2]:
            # [flip, no-flip] 
            self.writeline(f"# flip low-2d [flip, no-flip]")
            self.writeline(f"dst_offset += (num_y - pid_y - 1) * step * merge_shape_{self.merge_dim_size - 1}")
            self.writeline(f"if H_DIM != 0:")
            with self.indent():
                self.writeline(f"offset = tl.reshape(tl.arange(0, H_DIM*W_DIM), (H_DIM, W_DIM))")
                ...
                with self.indent():
                    ...
                    self.writeline(f"out_offset = dst_offset + tail * W_DIM + (iter - i - 1) * H_DIM*W_DIM")
                    self.writeline(f"src = tl.load(x_ptr + offset + in_offset, cache_modifier='.cg')")
                    self.writeline(f"src = tl.flip(src, [0])")
                    self.writeline(f"tl.store(out_ptr + offset + out_offset, src, cache_modifier='.cg')")
                ...
                with self.indent():
                    ...
                    self.writeline(f"src = tl.load(x_ptr + offset + in_offset, mask=mask, other=0.0, cache_modifier='.cg')")
                    self.writeline(f"src = tl.flip(src, [0])")
                    self.writeline(f"mask = offset >= (H_DIM - tail) * W_DIM")
                    self.writeline(f"tl.store(out_ptr + offset + out_offset, src, mask=mask, cache_modifier='.cg')")
            ...
            with self.indent():
                ...
                self.writeline(f"src = tl.zeros((W_DIM,), dtype=x_ptr.dtype.element_ty)")
                self.writeline(f"for i in range(0, step):")
                with self.indent():
                    self.writeline(f"in_offset = src_offset + i * merge_shape_{self.merge_dim_size-1}")
                    self.writeline(f"out_offset = dst_offset + (step - i - 1) * merge_shape_{self.merge_dim_size-1}")
                    self.writeline(f"for j in range(0, iter):")
                    with self.indent():
                        self.writeline(f"new_offset = offset + j*W_DIM")
                        self.writeline(f"src = tl.load(x_ptr + in_offset + new_offset, cache_modifier='.cg')")
                        self.writeline(f"tl.store(out_ptr + out_offset + new_offset, src, cache_modifier='.cg')")
                    self.writeline(f"if tail > 0:")
                    with self.indent():
                        self.writeline(f"new_offset = offset + iter*W_DIM")
                        self.writeline(f"mask = offset < tail")
                        self.writeline(f"src = tl.load(x_ptr + in_offset + new_offset, mask=mask, cache_modifier='.cg')")
                        self.writeline(f"tl.store(out_ptr + out_offset + new_offset, src, mask=mask, cache_modifier='.cg')")
        else:
            # [no-flip, flip]
            self.writeline(f"# flip low-2d [no-flip, flip]")
            self.writeline(f"dst_offset += pid_y * step * merge_shape_{self.merge_dim_size - 1}")
            self.writeline(f"if H_DIM != 0:")
            with self.indent():
                self.writeline(f"offset = tl.reshape(tl.arange(0, H_DIM*W_DIM), (H_DIM, W_DIM))")
                self.writeline(f"tail = step % H_DIM")
            ...
            with self.indent():
                ...
                with self.indent():
                    self.writeline(f"in_offset = src_offset + i * merge_shape_{self.merge_dim_size-1}")
                    self.writeline(f"out_offset = dst_offset + i * merge_shape_{self.merge_dim_size-1}")
                    self.writeline(f"if tail > 0:")
                    with self.indent():
                        self.writeline(f"new_offset = in_offset + iter * W_DIM")
                        self.writeline(f"mask = offset < tail")
                        self.writeline(f"src = tl.load(x_ptr + new_offset + offset, mask=mask, cache_modifier='.cg')")
                        self.writeline(f"src = tl.flip(src, [0])")
                        self.writeline(f"mask = offset >= (W_DIM-tail)")
                        self.writeline(f"tl.store(out_ptr + out_offset - (W_DIM - tail) + offset, src, mask=mask, cache_modifier='.cg')")
                    self.writeline(f"for j in range(0, iter):")
                    with self.indent():
                        self.writeline(f"new_in_offset = in_offset + j * W_DIM")
                        self.writeline(f"new_out_offset = tail + out_offset + (iter - j - 1) * W_DIM")
                        self.writeline(f"src = tl.load(x_ptr + new_in_offset + offset, cache_modifier='.cg')")
                        self.writeline(f"src = tl.flip(src, [0])")
                        self.writeline(f"tl.store(out_ptr + new_out_offset + offset, src, cache_modifier='.cg')")
                ...
                if self.merge_dim_size > 2:
                    for i in range(self.merge_dim_size - 2):
                        ...
                        if not self.merge_flip_dims_flags[i]:
                            ...
                            self.writeline(f"dst_offset += (merge_shape_{i} - id_{i} -1) * merge_stride_{i}")
                    ...
                    self.writeline(f"offset = tl.arange(0, W_DIM)")
                    self.writeline(f"step = merge_shape_{self.merge_dim_size - 1} // num_y")
                    self.writeline(f"tail = merge_shape_{self.merge_dim_size - 1} % num_y")
                    ...
                    with self.indent():
                        ...
                        self.writeline(f"out_offset = dst_offset + step_tail + (step_iter - i - 1) * W_DIM")
                        self.writeline(f"src = tl.load(x_ptr + offset + in_offset, cache_modifier='.cg')")
                        self.writeline(f"src = tl.flip(src, [0])")
                        self.writeline(f"tl.store(out_ptr + offset + out_offset, src, cache_modifier='.cg')")
                    ...
                    with self.indent():
                        ...
                        self.writeline(f"src = tl.load(x_ptr + offset + in_offset, mask=mask, cache_modifier='.cg')")
                        self.writeline(f"src = tl.flip(src, [0])")
                        self.writeline(f"mask = offset >= (W_DIM - step_tail)")
                        self.writeline(f"tl.store(out_ptr + offset + out_offset - (W_DIM - step_tail), src, mask=mask, cache_modifier='.cg')")
                    ...
                    with self.indent():
                        ...
                        with self.indent():
                            self.writeline(f"in_offset = src_offset + i * W_DIM")
                            self.writeline(f"out_offset = dst_offset + tail_remain + (tail_iter - i - 1) * W_DIM")
                            self.writeline(f"src = tl.load(x_ptr + offset + in_offset, cache_modifier='.cg')")
                            self.writeline(f"src = tl.flip(src, [0])")
                            self.writeline(f"tl.store(out_ptr + offset + out_offset, src, cache_modifier='.cg')")
                        self.writeline(f"if tail_remain > 0:")
                        with self.indent():
                            self.writeline(f"in_offset = src_offset + tail_iter * W_DIM")
                            self.writeline(f"out_offset = dst_offset")
                            self.writeline(f"mask = offset < tail_remain")
                            self.writeline(f"src = tl.load(x_ptr + offset + in_offset, mask=mask, cache_modifier='.cg')")
                            self.writeline(f"src = tl.flip(src, [0])")
                            self.writeline(f"mask = offset >= (W_DIM-tail_remain)")
                            self.writeline(f"tl.store(out_ptr + offset + out_offset - (W_DIM - tail_remain), src, mask=mask, cache_modifier='.cg')")
                else:
