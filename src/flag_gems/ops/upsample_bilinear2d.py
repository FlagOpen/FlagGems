...
def upsample_bilinear2d_kernel(
    ...
    Q11 = tl.load(input_bc_ptrs + y1 * input_stride_h + x1, mask=mask, other=0.0).to(
        tl.float32
    )
    Q21 = tl.load(input_bc_ptrs + y1 * input_stride_h + x2, mask=mask, other=0.0).to(
        tl.float32
    )
    Q12 = tl.load(input_bc_ptrs + y2 * input_stride_h + x1, mask=mask, other=0.0).to(
        tl.float32
    )
    Q22 = tl.load(input_bc_ptrs + y2 * input_stride_h + x2, mask=mask, other=0.0).to(
        tl.float32
    )
    # Calculate weights
    dx = x - x1
...
def upsample_bilinear2d(
    ...
    assert (
        input.dim() == 4
    ), f"Input must be 4D tensor (got {input.dim()}D) - (N, C, H, W)."
    if output_size is not None:
        assert (
            len(output_size) == 2
        ), f"size must have 2 elements (H, W), got {len(output_size)}."
        assert all(
            isinstance(d, int) and d > 0 for d in output_size
        ), f"Each element in size must be a positive integer, got {output_size}"
    if scale_factors is not None:
        assert all(
            isinstance(d, float) and d > 0 for d in scale_factors
        ), f"Each element in size must be a positive float, got {scale_factors}"
