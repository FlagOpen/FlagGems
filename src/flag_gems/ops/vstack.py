...
class VstackKernelCode(IndentedBuffer):
    ...
    def __init(self, tensors):
        ...
        if self.total_size < self.vstack_small_limit and self.input_num <= TOTAL_CORE_NUM and flag:
            self.is_small = True
        else:
            self.is_small = False
...
@triton.jit
            ...
            with self.indent():
                ...
                with self.indent():
                    for i in range(self.input_num):
                        ...
                        with self.indent():
                            self.writeline(f"for num in range(0, idx_{i+1} - idx_{i}, BLOCK_SIZE):")
                            with self.indent():
                                self.writeline("in_offset = num + block")
                                self.writeline(f"dst_offset = idx_{i} + num + block")
                                self.writeline(f"x = tl.load(input_{i} + in_offset, mask = in_offset < idx_{i+1} - idx_{i})")
                                self.writeline(f"tl.store(output + dst_offset, x, mask = dst_offset < idx_{i+1})")
            self.writeline("else:")
            with self.indent():
                    ...
                    with self.indent():
                        ...
                        with self.indent():
                            ...
                            with self.indent():
                                ...
                                self.writeline("for i in range(0, deal_num, BLOCK_SIZE):")
                                with self.indent():
                                    self.writeline("in_offset = offset + i + block")
                                    self.writeline("dst_offset = pid * deal_num + i + block")
                                    self.writeline("x = tl.load(input_0 + in_offset, mask=in_offset < idx_1 - idx_0)")
                                    self.writeline("tl.store(output + dst_offset, x, mask=dst_offset<idx_1)")
                            self.writeline("else:")
                            with self.indent():
                                for i in range(1, self.input_num, 1):
                                    ...
                                    with self.indent():
                                        self.writeline(f"offset = idx_{idx} - idx_{i} - condidate_num")
                                        self.writeline("if need_num != deal_num:")
                                        with self.indent():
                                            self.writeline("deal_rem = deal_num - per_fetch_num")
                                            self.writeline("for i in range(0, need_num, BLOCK_SIZE):")
                                            with self.indent():
                                                self.writeline("in_offset = offset + i + block")
                                                self.writeline("dst_offset = pid * deal_num + deal_rem + i + block")
                                                self.writeline(f"x = tl.load(input_{i} + in_offset, mask=in_offset < need_num)")
                                                self.writeline(f"tl.store(output + dst_offset, x, mask=dst_offset<idx_{i}+per_fetch_num)")
                                        self.writeline("else:")
                                        with self.indent():
                                            self.writeline("for i in range(0, need_num, BLOCK_SIZE):")
                                            with self.indent():
                                                self.writeline("in_offset = offset + i + block")
                                                self.writeline("dst_offset = pid * deal_num + i + block")
                                                self.writeline(f"x = tl.load(input_{i} + in_offset, mask=in_offset < idx_{idx}-idx_{i})")
                                                self.writeline(f"tl.store(output + dst_offset, x, mask=dst_offset<idx_{idx})")
    ...
    def __kernel_args(self, is_declare=True):
        ...
        if is_declare:
            ...
            return f"{extra_args_str}, output, total_size, input_num, deal_num, is_small"
    ...
    def __call__(self, tensors: list) -> torch.Tensor:
        ...
        return overload(tensors, self.inputs, self.idxs, self.total_size, self.input_num, self.deal_num, self.is_small)
