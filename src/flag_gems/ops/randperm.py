...
def radix_type_convert(k):
    if tl.constexpr(k.dtype == tl.int32):
...
def digit_hist_kernel(
    ...
    bin_segid = tl.program_id(0)
    pid1 = tl.program_id(1)
    grid1 = tl.num_programs(1)

    key_offset = pid1 * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    ...
    for p in range(passes):
        ...
        for s in range(bins_segment):
            ...
            # +1 for exclusive
            bin_offset = p * (bins + 1) * grid1 + (bin_id + 1) * grid1 + pid1
            # reduce rather than global atomic for perf issue
            tl.store(digit_hist + bin_offset, digit_sum)
            # zero init d_lookback
            tl.store(d_lookback + (p * bins + bin_id) * grid1 + pid1, 0)
        tl.store(digit_hist + p * (bins + 1) * grid1 + pid1, 0, mask=bin_segid == 0)
...
def radix_sortbykey_scatter_kernel(
    ...
    blk_bin_start = tl.program_id(0) * bins_segment
    for s in range(bins_segment):
        bin_id = s + blk_bin_start
        ...
        tl.store(
            d_lookback + p * grid1 * bins + pid1 * bins + bin_id,
            partial_counter,
            cache_modifier=".wt",
        )
        bin_offset = p * (bins + 1) + bin_id
        prefix_offsets = tl.load(digit_hist + bin_offset)
        bk = pid1 - 1
        inc_sum = bin_of_bucket
        while bk >= 0:
            rd_lbk_offset = p * grid1 * bins + bk * bins + bin_id
        ...
        global_offsets = (prefix_offsets + inc_sum - bin_of_bucket) + key_block_rank
...
def sort_by_key(key, value):
    ...
    if n_elements > 2 * 1024:
        ...
        grid = (bins // bins_per_sgement, triton.cdiv(n_elements, BLOCK_SIZE))
        digit_hist = torch.empty(
            (passes, bins + 1, grid[1]), dtype=torch.int32, device=key.device
        )
        d_lookback = torch.empty(
            bins * grid[1] * passes, dtype=torch.int32, device=key.device
        )
        ...
        digit_hist = torch.sum(digit_hist, dim=2, keepdim=False)
        # step2
        digit_hist = digit_hist.cumsum(dim=1)
        ...
        for p in range(passes):
            ...
            # step3
            with torch.cuda.device(key.device):
                radix_sortbykey_scatter_kernel[grid](
                    k_out,
                    v_out,
                    k_in,
                    v_in,
...
def randperm(
    ...
    *,
    ...
    i32max = torch.iinfo(torch.int32).max
    ...
    if n < i32max:
        ...
        rand_key = torch.randint(
            low=0, high=i64max, size=[n], dtype=torch.int64, device=device
        )
    perm_range = sort_by_key(rand_key, in_range)
    return perm_range
