...
@libentry()
@triton.autotune(
    configs=[triton.Config({"BLOCK_C": 2**n}, num_warps=1, num_stages=3)
...
@triton.autotune(
    configs=[
        ...
        for num in [4, 8, 16, 48] for s in [0, 3]
    ],
    key=["C"],
...
def celoss_indice_bwd_with_saved_sum_kernel(
    ...
    for batch_idx in range(batch_begin, batch_end):
        ...
        if TILE_MODE == 0:
            if C <= BLOCK_C:
                offset_c = tl.arange(0, C)
                single_celoss_indice_bwd(pid_n, offset_c, offset_d, final_max, final_sum,
                                         tgt, w_tgt, out_grad, mean_num, inp_ptr,
                                         inp_grad_ptr, ignore_mask, C, D)
            else:
                for off in range(0, C, BLOCK_C):
                    offset_c = off + tl.arange(0, BLOCK_C)
                    single_celoss_indice_bwd(pid_n, offset_c, offset_d, final_max, final_sum,
        ...
        if tgt.ndim == inp.ndim:
            ...
            if final_sum is not None:
                ...
                celoss_indice_bwd_with_saved_sum_kernel[grid](
                    out_grad, inp, tgt, weight, inp_grad,
                    final_max, final_sum, ignore_index, mean_num, N, C, D,
