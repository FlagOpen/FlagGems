...
def early_config_prune(configs, named_args, **kwargs):
    ...
    for config in configs:
        ...
        new_config = triton.Config({'BLOCK_M':block_m, 'BLOCK_N':block_n, 'NEED_LOOP_N': block_n < N}, num_stages=config.num_stages, num_warps=config.num_warps)
...
@triton.jit
def outer_kernel(lhs, rhs, res, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, NEED_LOOP_N: tl.constexpr):
    pid = tl.program_id(0)
    ...
    if NEED_LOOP_N:
        for task_id in range(pid, total_tasks_num, num_jobs):
            ...
            tl.store(res + offset_r, res_val, mask = (offset_m[:, None] < M) & (offset_n[None, :] < N))
    else:
...
def outer_(lhs, rhs):
    ...
    grid = lambda META: (min(triton.cdiv(m, META['BLOCK_M']) * 
                             triton.cdiv(n, META['BLOCK_N']), TOTAL_CORE_NUM),)
