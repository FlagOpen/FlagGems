...
at::Tensor zeros_tensor(
  ...
  at::Device final_device = device.value_or(
        torch::cuda::is_available() ? at::Device(at::kCUDA) : at::Device(at::kCPU)
    );
  at::Tensor out = at::empty({n_elements}, 
    at::TensorOptions().dtype(final_dtype).device(final_device));
